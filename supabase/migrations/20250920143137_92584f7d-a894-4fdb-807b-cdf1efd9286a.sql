...
-- تحديث صلاحيات الجرائم الإلكترونية الموجودة لتكون نشطة
UPDATE public.cybercrime_access 
SET is_active = true, granted_by = (SELECT user_id FROM public.profiles WHERE role = 'admin' AND is_active = true LIMIT 1)
WHERE user_id IN (
  SELECT user_id FROM public.profiles 
  WHERE role IN ('cybercrime', 'admin') AND is_active = true
);
