...
import { UserPlus, Car, Trash2, Search } from 'lucide-react';
...
const TrafficPoliceUsers = () => {
  ...
  useEffect(() => {
    ...
    const hasTrafficRole = hasRole('traffic_police');
    
    console.log('TrafficPoliceUsers - Permission check:', {
      isAdmin,
      hasTrafficRole,
      rolesLoading
    });

    if (!isAdmin && !hasTrafficRole) {
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      toast({
        title: 'نجح',
        description: 'تم إضافة المستخدم لشرطة المرور',
      });
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 bg-card px-4 md:px-6 py-3 rounded-full shadow-lg border">
            <Car className="h-6 md:h-8 w-6 md:w-8 text-blue-600" />
            <h1 className="text-xl md:text-3xl font-bold">مستخدمي شرطة المرور</h1>
          </div>
          <div />
        </div>
...
export default TrafficPoliceUsers;
