...
const SpecialPoliceUsers = () => {
  const navigate = useNavigate();
  const { toast } = useToast();
  const { isAdmin, hasRole, loading: rolesLoading } = useUserRoles();
  const [users, setUsers] = useState<SpecialPoliceUser[]>([]);
  ...
  useEffect(() => {
    ...
    const hasSpecialPoliceRole = hasRole('special_police');
    
    if (!isAdmin && !hasSpecialPoliceRole) {
      navigate('/access-denied');
      return;
    }

    fetchSpecialPoliceUsers();
    fetchAllProfiles();
  }, [isAdmin, rolesLoading, navigate]);

  const fetchSpecialPoliceUsers = async () => {
    try {
      const { data: rolesData, error: rolesError } = await supabase
        .from('user_roles')
        .select('id, user_id, role, created_at')
        .eq('role', 'special_police')
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      toast({
        title: 'نجح',
        description: 'تم إضافة المستخدم للشرطة الخاصة',
      });
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 bg-card px-4 md:px-6 py-3 rounded-full shadow-lg border">
            ...
            <h1 className="text-xl md:text-3xl font-bold">مستخدمي الشرطة الخاصة</h1>
          </div>
          <div />
        </div>
...
export default SpecialPoliceUsers;
