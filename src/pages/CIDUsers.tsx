...
const CIDUsers = () => {
  ...
  const [users, setUsers] = useState<CIDUser[]>([]);
  ...
  useEffect(() => {
    ...
    const hasCIDRole = hasRole('cid');
    
    if (!isAdmin && !hasCIDRole) {
  ...
  const handleAddUser = async () => {
    ...
    try {
      ...
      toast({
        title: 'نجح',
        description: 'تم إضافة المستخدم للمباحث الجنائية',
      });
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 bg-card px-4 md:px-6 py-3 rounded-full shadow-lg border">
            <Shield className="h-6 md:h-8 w-6 md:w-8 text-purple-600" />
            <h1 className="text-xl md:text-3xl font-bold">مستخدمي المباحث الجنائية</h1>
          </div>
          <div />
        </div>
